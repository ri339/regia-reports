...
export default function App() {
  ...
  const upd  = (key, val) => setReport(r => ({ ...r, cover: { ...r.cover, [key]: val } }))
  const updB = (key, val) => setReport(r => ({ ...r, backCover: { ...r.backCover, [key]: val } }))
  const updS = (id, key, val) =>
    setReport(r => ({ ...r, sections: r.sections.map(s => s.id === id ? { ...s, [key]: val } : s) }))

  const addSection = (type = 'content') => {
    const base = { id: uid(), type, title: '', subtitle: '', body: '', image: null, number: '', headerLabel: '', imagePosition: 'bottom', layout: type === 'two-col' ? 'two-col' : 'default' }
    const extras =
      type === 'highlight'  ? { highlights: [{ value: '', label: '' }, { value: '', label: '' }, { value: '', label: '' }, { value: '', label: '' }] } :
  ...
  const handleImport = async e => {
    ...
    try {
      ...
      if (data.sections?.length) {
        ...
        setActiveSec(data.sections[0]?.id)
        setTab('sections')
      } else {
        setImportMsg('Nenhuma seção detectada. Verifique o arquivo.')
      }
    } catch { setImportMsg('Erro ao importar. Servidor ativo?') }
    setImporting(false)
    e.target.value = ''
  }
  ...
  const handlePDF = async () => {
    ...
    try {
      ...
      a.href = url
      a.download = `relatorio-regia-${Date.now()}.pdf`
      a.click()
  ...
  return (
    <div style={S.shell}>
        ...
        <div style={{ ...S.sideSection, flex: 1, overflowY: 'auto' }}>
          ...
          {report.sections.map((s, i) => (
            <div key={s.id} style={S.secRow}>
              <button
                style={{ ...S.secBtn, ...(activeSec === s.id && tab === 'sections' ? S.secBtnActive : {}) }}
                onClick={() => { setActiveSec(s.id); setTab('sections') }}
              >
                <span style={S.secNum}>{i + 1}</span>
                <span style={S.secTitle}>{s.title || `Seção ${i + 1}`}</span>
                <span style={S.secType}>{TYPES.find(t => t.value === s.type)?.label?.split(' ')[0]}</span>
              </button>
      ...
      {/* ── EDITOR ── */}
      <main style={S.editor}>
        {tab === 'cover'    && <CoverEditor cover={report.cover} onChange={upd} toBase64={toBase64} />}
        {tab === 'sections' && activeSection
          ? <SectionEditor section={activeSection} onChange={(k,v) => updS(activeSection.id,k,v)} toBase64={toBase64} />
          : tab === 'sections' && <Empty onAdd={() => addSection()} />
        }
        {tab === 'backcover' && <BackCoverEditor bc={report.backCover} onChange={updB} />}
      </main>

      {/* ── PREVIEW ── */}
      <aside style={S.preview}>
        <div style={S.previewHdr}>Prévia</div>
        <Preview report={report} tab={tab} activeSec={activeSec} />
      </aside>
    </div>
  )
}
  ...
  return (
    <Panel title="Capa">
      <Field label="Logo (PNG/SVG)">
        <ImgUpload value={cover.logo} onUpload={async f => onChange('logo', await toBase64(f))} onClear={() => onChange('logo', null)} />
      </Field>
      <Field label="Título principal">
        <Textarea value={cover.title} onChange={v => onChange('title', v)} rows={3} placeholder="Ex: Climate and Biodiversity Report 2024" />
      </Field>
      <Field label="Subtítulo / Período">
        <Input value={cover.subtitle} onChange={v => onChange('subtitle', v)} placeholder="Ex: February 2026" />
      </Field>
      <Field label="Eyebrow (texto acima do título)">
        <Input value={cover.eyebrow} onChange={v => onChange('eyebrow', v)} placeholder="Ex: ESG · Relatório Anual" />
      </Field>
      <Field label="Empresa / Cliente">
        <Input value={cover.client} onChange={v => onChange('client', v)} placeholder="Régia Capital" />
      </Field>
      <Field label="Data">
        <Input value={cover.date} onChange={v => onChange('date', v)} placeholder="Junho 2025" />
      </Field>
...
function SectionEditor({ section, onChange, toBase64 }) {
  ...
  const updHL = (i, k, v) => {
    const a = [...(section.highlights || [])]
    a[i] = { ...a[i], [k]: v }
    onChange('highlights', a)
  }
  const addHL = () => onChange('highlights', [...(section.highlights || []), { value: '', label: '' }])
  const delHL = i => { const a = [...(section.highlights || [])]; a.splice(i, 1); onChange('highlights', a) }

  const updItem = (i, k, v) => {
    const a = [...(section.items || [])]
    a[i] = { ...a[i], [k]: v }
    onChange('items', a)
  }
  const addItem = () => onChange('items', [...(section.items || []), { name: '', page: '' }])
  const delItem = i => { const a = [...(section.items || [])]; a.splice(i, 1); onChange('items', a) }
  ...
  return (
    <Panel title="Editar Seção" tag={typeLabel}>
      {!isSum && (
        <>
      ...
      {isContent && (
        <>
          <Field label="Subtítulo">
            <Input value={section.subtitle||''} onChange={v=>onChange('subtitle',v)} placeholder="Subtítulo ou descrição" />
          </Field>
          <Field label="Label no cabeçalho">
            <Input value={section.headerLabel||''} onChange={v=>onChange('headerLabel',v)} placeholder="Ex: Governança" />
          </Field>
          ...
          {type !== 'two-col' && (
            <Field label="Posição da imagem">
              <select style={S.select} value={section.imagePosition||'bottom'} onChange={e=>onChange('imagePosition',e.target.value)}>
      ...
      {isFI && (
        <>
          <Field label="Imagem de fundo">
            <ImgUpload value={section.image} onUpload={async f=>onChange('image',await toBase64(f))} onClear={()=>onChange('image',null)} />
          </Field>
          <Field label="Texto sobreposto (legenda)">
            <Textarea value={section.caption||''} onChange={v=>onChange('caption',v)} rows={3} placeholder="Texto sobre a imagem" />
          </Field>
        </>
      )}
        ...
        <Field label="KPIs / Destaques">
          {(section.highlights||[]).map((h,i) => (
            <div key={i} style={S.hlRow}>
              ...
              <IBtn red onClick={()=>delHL(i)}>✕</IBtn>
            </div>
          ))}
          <button style={S.addRowBtn} onClick={addHL}>+ Adicionar KPI</button>
        </Field>
      )}
          ...
          <Field label="Itens">
            {(section.items||[]).map((item,i) => (
              <div key={i} style={S.hlRow}>
                ...
                <IBtn red onClick={()=>delItem(i)}>✕</IBtn>
              </div>
            ))}
            <button style={S.addRowBtn} onClick={addItem}>+ Adicionar item</button>
          </Field>
        </>
      )}
    </Panel>
...
function BackCoverEditor({ bc, onChange }) {
  return (
    <Panel title="Contracapa">
      <Field label="Título"><Input value={bc.title} onChange={v=>onChange('title',v)} placeholder="regiacapital.com.br" /></Field>
      <Field label="Texto de encerramento"><Textarea value={bc.text} onChange={v=>onChange('text',v)} rows={5} placeholder="Texto opcional de encerramento" /></Field>
      ...
      <Field label="Cor de fundo"><ColorPick value={bc.backgroundColor} onChange={v=>onChange('backgroundColor',v)} /></Field>
    </Panel>
  )
}

// ── PREVIEW ───────────────────────────────────────────────────────────────────
function Preview({ report, tab, activeSec }) {
  const { cover, sections, backCover } = report
  const bg    = cover.backgroundColor || DARK
  const backBg = backCover.backgroundColor || bg

  const pg = { width:'100%', aspectRatio:'1/1.414', borderRadius:3, overflow:'hidden', marginBottom:8, border:'1px solid #dddbd8', position:'relative', flexShrink:0, background:'#fff' }
  ...
  return (
    <div style={S.previewScroll}>
      ...
      <div style={{ ...pg, background: bg, ...(tab==='cover' ? act : {}) }}>
        <div style={{ padding:'12% 14%', height:'100%', display:'flex', flexDirection:'column', justifyContent:'space-between' }}>
          ...
          <div>
            ...
            <div style={{ fontSize:10, color:'#fff', lineHeight:1.1, fontWeight:300 }}>{cover.title||'Título'}</div>
      ...
      {sections.map(s => {
        ...
        return (
          <div key={s.id} style={{ ...pg, background:sBg, ...(activeSec===s.id?act:{}) }}>
            <div style={{ padding:'10% 12%', height:'100%' }}>
              ...
              {s.type==='highlight' && (
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:2, height:'100%' }}>
                  {(s.highlights||[]).slice(0,4).map((h,i) => (
                    <div key={i} style={{ background:'rgba(255,255,255,.05)', display:'flex', flexDirection:'column', justifyContent:'flex-end', padding:6 }}>
                ...
                <div>
                  ...
                  {(s.items||[]).slice(0,10).map((item,i)=>(
                    <div key={i} style={{ display:'flex', justifyContent:'space-between', borderBottom:'1px solid #e5e5e2', padding:'2.5px 0', fontSize:4.5, color:'#555' }}>
                      <span>{item.name}</span><span style={{ color:'#bbb' }}>{item.page}</span>
                    </div>
                  ))}
                </div>
              )}
              {(s.type==='content'||s.type==='two-col') && (
                <>
                  ...
                  {s.image && s.imagePosition==='top' && <div style={{ width:'100%', height:20, background:'#e8e8e5', borderRadius:2, marginBottom:5 }}></div>}
                  {s.title && <div style={{ fontSize:7, color:'#0e1820', lineHeight:1.2, marginBottom:3 }}>{s.title.slice(0,40)}</div>}
                  {s.image && s.imagePosition!=='top' && !s.layout?.includes('two-col') && <div style={{ width:'100%', height:16, background:'#e8e8e5', borderRadius:2, marginTop:5 }}></div>}
                </>
              )}
            </div>
          </div>
      ...
      {/* Contracapa */}
      <div style={{ ...pg, background:backBg, ...(tab==='backcover'?act:{}) }}>
        <div style={{ padding:'12% 14%', height:'100%', display:'flex', flexDirection:'column', justifyContent:'space-between' }}>
  ...
  return (
    <div style={{ display:'flex', gap:8, alignItems:'center' }}>
      <input type="color" value={value||'#0f1923'} onChange={e=>onChange(e.target.value)}
        style={{ width:38, height:34, border:'none', borderRadius:4, cursor:'pointer', padding:0, background:'none' }} />
      <input style={{ ...S.input, flex:1, fontFamily:'monospace', fontSize:12 }} value={value||''} onChange={e=>onChange(e.target.value)} placeholder="#0f1923" />
    </div>
  )
}
    ...
    <div>
      {value && (
        <div style={S.imgWrap}>
          ...
          {onClear && <button style={S.imgClear} onClick={onClear}>Remover</button>}
        </div>
      )}
      <label style={S.uploadBtn}>
        {value ? 'Trocar imagem' : 'Enviar imagem'}
        <input type="file" accept="image/*" onChange={e=>{ const f=e.target.files?.[0]; if(f)onUpload(f); e.target.value='' }} style={{ display:'none' }} />
      </label>
    </div>
  )
}
function NavBtn({ active, onClick, children }) {
  return <button style={{ ...S.navBtn, ...(active?S.navBtnActive:{}) }} onClick={onClick}>{children}</button>
}
...
const S = {
  ...
  sideSection: { padding:'12px 14px', borderBottom:'1px solid rgba(255,255,255,.06)' },
  ...
  navBtnActive:{ background:'rgba(255,255,255,.09)', color:'#fff' },
  ...
  secBtnActive:{ background:'rgba(255,255,255,.09)', color:'#fff' },
  secNum:      { color:'rgba(255,255,255,.2)', fontSize:9.5, width:14, flexShrink:0 },
  secTitle:    { flex:1, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' },
  secType:     { fontSize:8.5, color:'rgba(255,255,255,.18)', flexShrink:0 },
  secActions:  { display:'flex', gap:2, paddingLeft:28 },
  ...
  previewHdr:  { padding:'14px 16px 10px', fontSize:10, letterSpacing:'.12em', textTransform:'uppercase', color:'#aaa', borderBottom:'1px solid #dddbd8', flexShrink:0 },
  previewScroll:{ flex:1, overflowY:'auto', padding:'12px 12px 24px' },
}
